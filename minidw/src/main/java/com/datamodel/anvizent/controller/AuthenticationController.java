...
public class AuthenticationController
{
	...
	public ModelAndView getLoginPage(HttpServletRequest request, HttpServletResponse response, ModelAndView mv, @ModelAttribute("loginForm") @Valid LoginForm loginForm, BindingResult result, Locale defaultLocale, RedirectAttributes redirectAttributes)
	{
		...
		try
		{
			return securityCheckProcess(request, response, isWebApp, redirectAttributes, mv, username, password, clientID, accessKey, timeZone, "login", lcoale);
		}
		catch ( Throwable e )
		{
			redirectAttributes.addFlashAttribute("errors", "Unable to setup environment. Please contact admin");
	...
	@SuppressWarnings("unchecked")
	private ModelAndView securityCheckProcess(HttpServletRequest request, HttpServletResponse response, Boolean isWebApp, RedirectAttributes redirectAttributes, ModelAndView mv, String username, String password, String clientID, String accessKey, String timeZone, String sourceFrom, String locale)
	{
		...
		DataResponse dataResponse = loginServicesRestTemplateUtilities.postRestObject(request, "/authetincatUserCredentials", user);
	...
	public ModelAndView autoLogin(HttpServletRequest request, HttpServletResponse response, ModelAndView mv, @RequestParam("auth_token") String auth_token, @RequestParam(value = "locale", required = false) String locale, RedirectAttributes redirectAttributes)
	{
		...
		if( credentials != null && credentials.length == 4 )
		{
			...
			Boolean isWebApp = Boolean.parseBoolean(request.getServletContext().getAttribute("isWebApp").toString());
			return securityCheckProcess(request, response, isWebApp, redirectAttributes, mv, userName, password, clientId, null, request.getParameter("time_zone"), "auto", locale);
		}
		else
		{
