...
public class ExecutionProcessor {
	...
	public DataResponse runIl(User user, Modification modification, String timeZone, String jobFilesPath,
			ClientJdbcInstance clientJdbcInstance, PackageExecutorMappingInfo packageExecutorMappingInfo,
			String deploymentType, Package userPackage) throws TalendJobNotFoundException, CSVConversionException,
			InterruptedException, IOException, AnvizentCorewsException {
		...
		try {
			...
			iLConnectionMapping = packageDao.getIlConnectionMappingByPackageExecutorSourceMappingInfo(
					packageExecutorMappingInfo.getExecutionId(), packageExecutorMappingInfo.getId(),
					clientAppDbJdbcTemplate);
			
			if (iLConnectionMapping != null) {
				
				String defaultSrcTimezone = null;
				ILConnectionMapping iLConnectionMappingInfobyConnectionFlags = packageDao
						.getILConnectionMappingInfoForPreview(iLConnectionMapping.getConnectionMappingId(),
								iLConnectionMapping.getPackageId(), user.getClientId(), clientAppDbJdbcTemplate);
				...
				if (iLConnectionMappingInfobyConnectionFlags.getIsWebservice() != null
						&& iLConnectionMappingInfobyConnectionFlags.getIsWebservice()) {
					WebServiceConnectionMaster webServiceConnectionMaster = webServiceDao.getWebServiceConnectionDetails(
							Long.valueOf(iLConnectionMappingInfobyConnectionFlags.getWsConId()), user.getClientId(),
							clientAppDbJdbcTemplate);
					...
					if (iLConnectionMappingInfobyConnectionFlags.getIsFlatFile()) {
						...
						ILConnection iLConnection = packageDao.getILConnectionById(
								iLConnectionMappingInfobyConnectionFlags.getiLConnection().getConnectionId(),
								user.getUserId(), clientAppDbJdbcTemplate);
						defaultSrcTimezone = iLConnection.getTimeZone();
						datbaseTypeName = "D"
								+ iLConnectionMappingInfobyConnectionFlags.getiLConnection().getConnectionId();
					}
				}
				datbaseTypeName = StringUtils.isNotBlank(iLConnectionMapping.getIlSourceName())
						? iLConnectionMapping.getIlSourceName() : datbaseTypeName;
				...
				ILInfo iLInfo = packageDao.getILByIdWithJobName(iLConnectionMapping.getiLId(), user.getClientId(),
						clientAppDbJdbcTemplate);
				if (iLInfo != null) {
					...
					if (deploymentType.equals(Constants.Config.DEPLOYMENT_TYPE_ONPREM)) {
						...
						s3BucketInfo = userDao.getS3BucketInfoById(user.getClientId(),
								iLConnectionMapping.getS3BucketId(), null);
					}
					if (iLInfo.getJobExecutionType().equals("E")) {

						String messageString = "IL %s execution started for execution source mapping id %d";
						messageString = String.format(messageString, iLInfo.getiL_name(),
								packageExecutorMappingInfo.getId());
						metaDataFetch.updateExecutionInfo(Constants.ExecutionStatus.INPROGRESS, messageString,
								packageExecutorMappingInfo.getPackageExecution(),
								getCustomRequest(packageExecutorMappingInfo.getPackageExecution().getClientId()));
						
						JSONObject externalDataJSON  = commonProcessor.getExternalDataJSON(packageExecutorMappingInfo.getId(),packageExecutorMappingInfo.getPackageExecution().getScheduleId(),
								packageExecutorMappingInfo.getPackageExecution().getExecutionId(),packageExecutorMappingInfo.getPackageExecution().getClientId(),
								packageExecutorMappingInfo.getPackageExecution().getDlId(),packageExecutorMappingInfo.getPackageExecution().getIlId());
						
						try {
							Map<String,Object> executionResponse = eltProcessor.processEltJob(iLInfo.getJobTagId(), iLInfo.getLoadParameterId(),
									iLInfo.getMasterParameterId(), packageExecutorMappingInfo.getPackageExecution(),
									true, iLConnectionMapping.isMultipartEnabled(), iLConnectionMapping.getFilePath(),
									s3BucketInfo, datbaseTypeName,externalDataJSON,deploymentType,modification,clientJdbcInstance);
							...
							// incremental update
							if (iLConnectionMapping.getIsIncrementalUpdate()
									&& StringUtils.isNotBlank(iLConnectionMapping.getIncrementalDateValue())) {
								clientStagingJdbcTemplate = clientJdbcInstance.getClientJdbcTemplate(true);
								scheduleDao.saveOrUpdateIncrementalUpdate(
										iLConnectionMapping.getIncrementalDateValue(),
										iLConnectionMapping.getConnectionMappingId(), clientAppDbJdbcTemplate,
							...
							messageString = String.format(messageString, iLInfo.getiL_name(),
									packageExecutorMappingInfo.getId(), e.getMessage());
							metaDataFetch.updateExecutionInfo(Constants.ExecutionStatus.INPROGRESS, messageString,
									packageExecutorMappingInfo.getPackageExecution(),
									getCustomRequest(packageExecutorMappingInfo.getPackageExecution().getClientId()));
						}
					...
					packageExecution = commonProcessor.packageExecutionMappingInfo(Constants.ExecutionStatus.STARTED,
							"Il execution started.", packageExecutorMappingInfo.getId(), modification, timeZone);
					...
					if (s3BucketInfo != null) {
						...
						filePaths = s3FileProcessor.downloadFilesFromS3(iLConnectionMapping.getFilePath(),
								user.getUserId(), deploymentType, iLConnectionMapping.isMultipartEnabled(),
								iLConnectionMapping.isEncryptionRequired(), s3BucketInfo);
						if (filePaths != null) {
							Map<String, String> ilContextParams = packageDao
									.getIlContextParams(iLConnectionMapping.getiLId(), clientAppDbJdbcTemplate);
							clientJobExecutionParameters = packageDao
									.getClientJobExecutionParams(clientAppDbJdbcTemplate);
							...
							Map<String, String> contextParams = preparedObjectProcessor.getContextParams(
									ilContextParams, databaseHost, databasePort, clientStagingSchema, databaseUserName,
									databasePassword, databaseHost, databasePort, clientSchemaName, databaseUserName,
									databasePassword, databaseHost, databasePort, clientStagingSchema, databaseUserName,
									databasePassword, datbaseTypeName, iLInfo, user.getClientId(),
									userPackage.getPackageId(), jobFilesPath,
									packageExecutorMappingInfo.getExecutionId(), clientJobExecutionParameters,
									iLConnectionMapping.getIsIncrementalUpdate(),
									packageExecutorMappingInfo.getPackageExecution().getDlId());
							if (contextParams != null) {
								executionMappingInfoComments.append(Constants.Config.NEW_LINE)
										.append(getFormattedDateString()).append("\t IL job started...")
										.append(Constants.Config.NEW_LINE)
										.append("id : " + iLInfo.getiL_id() + "; IL Name :" + iLInfo.getiL_name());
								int ilSourceIterationCount = 1;
								int ilSourceMultiPartCount = filePaths.size();
								log.info("IL job started...id : " + iLInfo.getiL_id() + "; IL Name :"
										+ iLInfo.getiL_name() + "; IL Job Class :" + iLInfo.getJobName());
								executionMappingInfoComments.append(Constants.Config.NEW_LINE)
								.append(getFormattedDateString())
								.append("\t Execution started for the partfiles ")
								.append(ilSourceMultiPartCount).append("\n");
								...
								for (String filePath : filePaths) {
									...
									packageExecution = commonProcessor.getExecutionStatus(
											packageExecutorMappingInfo.getExecutionId(),
											Constants.ExecutionStatus.INPROGRESS, statusString, timeZone);
									packageExecution.setModification(modification);
									scheduleDao.updatePackageExecutionStatusInfo(packageExecution,
											clientAppDbJdbcTemplate);
									executionMappingInfoComments = new StringBuilder();
									jobExecutionInfo = etlJobProcessor.runILEtlJob(contextParams, iLInfo,
											user.getUserId(), userPackage.getPackageId(), filePath, modification,
											jobFilesPath, user);
									scheduleDao.updateJobExecutionDetails(jobExecutionInfo, clientAppDbJdbcTemplate);
									executionMappingInfoComments.append(Constants.Config.NEW_LINE)
											.append(jobExecutionInfo.getExecutionMessages());
									executionMappingInfoComments.append(Constants.Config.NEW_LINE)
											.append(getFormattedDateString())
											.append("\t Execution completed for the partfile ")
											.append(ilSourceIterationCount).append(" of ")
											.append(ilSourceMultiPartCount);
								...
								log.info("IL job completed...id : " + iLInfo.getiL_id() + "; IL Name :"
										+ iLInfo.getiL_name());
								executionMappingInfoStatus = Constants.ExecutionStatus.COMPLETED;

								packageExecution = commonProcessor.packageExecutionMappingInfo(
										executionMappingInfoStatus, "Il execution completed.",
										packageExecutorMappingInfo.getId(), modification, timeZone);
								packageExecution.setModification(modification);
								scheduleDao.updatePackageExecutionMappingInfoStatus(packageExecution,
										clientAppDbJdbcTemplate);
							...
							message.setCode("ERROR");
							message.setText(
									"files not found in s3 for buket id: " + iLConnectionMapping.getS3BucketId());
						}
					} else {
						message.setCode("ERROR");
						message.setText(
								"s3 bucket info not found for buket id: " + iLConnectionMapping.getS3BucketId());
					}
				} else {
					message.setCode("ERROR");
				...
				message.setCode("ERROR");
				message.setText("Mapping info not found for execution id:" + packageExecutorMappingInfo.getExecutionId()
						+ " with mapping id:" + packageExecutorMappingInfo.getId());
			}
		} catch (Throwable e) {
			log.error("", e);
			message.setCode("ERROR");
	...
	public DataResponse runDl(User user, Modification modification, String timeZone, String jobFilesPath,
			ClientJdbcInstance clientJdbcInstance, long executionId,
			String deploymentType, Package userPackage, Integer dLId) throws TalendJobNotFoundException,
			CSVConversionException, InterruptedException, IOException, AnvizentCorewsException, ParseException {
		DataResponse dataResponse = new DataResponse();
		List<Message> messages = new ArrayList<>();
		Message message = new Message();
		messages.add(message);
		...
		try {
			...
			if (dLInfo != null) {
				...
				if (dLInfo.getJobExecutionType().equals("E")) {
					...
					metaDataFetch.updateExecutionInfo(Constants.ExecutionStatus.INPROGRESS, messageString,
							packageExecution, getCustomRequest(packageExecution.getClientId()));
					...
					try {
						eltProcessor.processEltJob(dLInfo.getJobTagId(), dLInfo.getLoadParameterId(),
								dLInfo.getMasterParameterId(), packageExecution, externalDataJSON,deploymentType,modification,clientJdbcInstance);
				...
				Map<String, String> dlParamsVals = preparedObjectProcessor.getDlParamsVals(databaseHost, databasePort,
						...
						clientStagingSchema, databaseUserName, databasePassword, user.getClientId(), userPackage,
						jobFilesPath, executionId, clientJobExecutionParameters, loadType, dLId);

				if (dlParamsVals != null) {
					// run DL Jobs

					Map<String, String> dlContextParams = packageDao.getDlContextParams(dLInfo.getdL_id(),
							clientAppDbJdbcTemplate);
					if (dlContextParams != null) {
						...
						executionMappingInfoComments.append(Constants.Config.NEW_LINE).append("DL job started...")
								.append("id : " + dLInfo.getdL_id() + "; DL Name :" + dLInfo.getdL_name()
										+ "; DL Job Name :" + dLInfo.getJobName())
								.append(" date and time: ").append(startDate);
						executionMappingInfoComments.append("\nContext Parameters \n")
						.append(getFormattedDateString()).append("\t ").append(commonProcessor.getPrintableParams(dlContextParams));
						metaDataFetch.updateExecutionInfo(Constants.ExecutionStatus.INPROGRESS, executionMappingInfoComments.toString(),
								packageExecution, getCustomRequest(packageExecution.getClientId()));
						
						
						
						executionMappingInfoComments = new StringBuilder();
						jobExecutionInfo = etlJobProcessor.runDlEtlJob(dlContextParams, dlParamsVals, dLInfo,
								user.getUserId(), userPackage, modification, jobFilesPath, user);
						endDate = getFormattedDateString();

						updatePackageExecutionTargetTableInfo(dLInfo.getdL_table_name(), startDate, endDate,
								jobExecutionInfo, packageDao, executionId, modification, clientAppDbJdbcTemplate);
						scheduleDao.updateJobExecutionDetails(jobExecutionInfo, clientAppDbJdbcTemplate);
						executionMappingInfoComments.append(Constants.Config.NEW_LINE).append("DL job completed...")
								.append("id : " + dLInfo.getdL_id() + "; DL Name :" + dLInfo.getdL_name())
								.append(" date and time: ").append(startDate);
						;
						executionMappingInfoComments.append(Constants.Config.NEW_LINE)
						...
						for (DLInfo anly_DL : anlyDlList) {
							...
							startDate = getFormattedDateString();
							executionMappingInfoComments.append(Constants.Config.NEW_LINE)
									.append("Started date and time: ").append(startDate)
									.append("\t Analytical DL job started...").append(Constants.Config.NEW_LINE)
									.append("id : " + anly_DL.getdL_id() + "; DL Name :" + anly_DL.getdL_name());
							jobExecutionInfo = etlJobProcessor.runAnalyticalDlEtlJob(dlContextParams, anly_DL,
									user.getUserId(), userPackage, modification, jobFilesPath, user);
							endDate = getFormattedDateString();
							updatePackageExecutionTargetTableInfo(anly_DL.getdL_table_name(), startDate, endDate,
									jobExecutionInfo, packageDao, executionId, modification, clientAppDbJdbcTemplate);
							...
							executionMappingInfoComments.append(Constants.Config.NEW_LINE)
									.append(jobExecutionInfo.getExecutionMessages());
							executionMappingInfoComments.append(Constants.Config.NEW_LINE)
									.append("Started completed and time: ").append(endDate)
									.append("\t Analytical DL job completed...").append(Constants.Config.NEW_LINE)
									.append("id : " + anly_DL.getdL_id() + "; DL Name :" + anly_DL.getdL_name());
						}
						PackageExecution packageExecution1 = commonProcessor.getExecutionStatus(executionId,
								Constants.ExecutionStatus.INPROGRESS, executionMappingInfoComments.toString(),
								timeZone);
	...
	public int updatePackageExecutionTargetTableInfo(String targetTableName, String startDate, String endDate,
			JobExecutionInfo jobExecutionInfo, PackageDao packageDao, long executionId, Modification modification,
	...
	public DataResponse runCustomDataSetsInStandardPackage(Set<Integer> ddLTableSet, User user,
			JdbcTemplate jdbcTemplate, JdbcTemplate clientAppDbJdbcTemplate, Modification modification,
			String runType) throws SQLException {
		DataResponse dataResponse = new DataResponse();
		List<Message> messages = new ArrayList<>();
		Message message = new Message();
		List<String> ddlsTable = new ArrayList<>();
		try {
			List<DDLayout> dDLayoutList = packageDao.getDDlayoutListByIds(user.getClientId(), ddLTableSet,
					user.getUserId(), clientAppDbJdbcTemplate);
			if (dDLayoutList != null && dDLayoutList.size() > 0) {
				for (DDLayout ddlayout : dDLayoutList) {
					boolean tableExist = fileDao.isTableExists(user.getUserId(), null, ddlayout.getTableName(),
							jdbcTemplate);
					if (tableExist) {
						int count = 0;
						String errorMessage = "";
	...
	public DataResponse runCustomDataSetsInCustomPacakge(PackageDao packageDao, List<String> tableList, User user,
			JdbcTemplate jdbcTemplate, JdbcTemplate clientAppdbJdbcTemplate, FileDao fileDao, Modification modification,
			String runType) throws SQLException {
		DataResponse dataResponse = new DataResponse();
		List<Message> messages = new ArrayList<>();
		Message message = new Message();
		...
		try {
			message.setCode("SUCCESS");
			List<DDLayout> dDLayoutList = packageDao.getDDlayoutList(user.getClientId(), tableList, user.getUserId(),
					clientAppdbJdbcTemplate);
			if (dDLayoutList != null && dDLayoutList.size() > 0) {
				for (DDLayout ddlayout : dDLayoutList) {
					boolean tableExist = fileDao.isTableExists(user.getUserId(), null, ddlayout.getTableName(),
	...
	public DataResponse runHistoricalLoad(User user, String jobFilesPath, ClientJdbcInstance clientJdbcInstance,
			ScheduleDao scheduleDao, PackageDao packageDao, HistoricalLoadForm historicalLoadForm,
			String deploymentType, S3BucketInfo s3BucketInfo, ILInfo iLInfo, String filePath, FileSettings fileSettings)
			throws TalendJobNotFoundException, CSVConversionException, InterruptedException, IOException,
			AnvizentCorewsException, ParseException {
		DataResponse dataResponse = new DataResponse();
		List<Message> messages = new ArrayList<>();
		Message message = new Message();
		...
		try {
			...
			filePaths = s3FileProcessor.downloadFilesFromS3(filePath, user.getUserId(), deploymentType,
					historicalLoadForm.isMultipartEnabled(), fileSettings.getFileEncryption(), s3BucketInfo);
			if (filePaths != null) {
				Map<String, String> ilContextParams = packageDao.getIlContextParams(iLInfo.getiL_id(),
						clientAppDbJdbcTemplate);
				clientJobExecutionParameters = packageDao.getClientJobExecutionParams(clientAppDbJdbcTemplate);
				...
				Map<String, String> contextParams = MinidwServiceUtil.getContextParams(ilContextParams, databaseHost,
						databasePort, clientStagingSchema, databaseUserName, databasePassword, databaseHost,
						databasePort, clientSchemaName, databaseUserName, databasePassword, databaseHost, databasePort,
						clientStagingSchema, databaseUserName, databasePassword, datbaseTypeName, iLInfo,
						user.getUserId(), iLInfo.getiL_id(), jobFilesPath, historicalLoadForm.getId(),
						clientJobExecutionParameters, false);
				if (contextParams != null) {
					executionComments.append(Constants.Config.NEW_LINE).append("Historical IL job started...")
							.append(Constants.Config.NEW_LINE)
							.append("id : " + iLInfo.getiL_id() + "; IL Name :" + iLInfo.getiL_name());
					int ilSourceIterationCount = 1;
					int ilSourceMultiPartCount = filePaths.size();
					executionComments.append(Constants.Config.NEW_LINE).append("Execution started for the partfiles ")
							.append(ilSourceMultiPartCount);
					for (String path : filePaths) {
						jobExecutionInfo = etlJobProcessor.runILEtlJob(contextParams, iLInfo, user.getUserId(),
								historicalLoadForm.getId(), path, modification, jobFilesPath, user);
						executionComments.append(Constants.Config.NEW_LINE)
								.append(jobExecutionInfo.getExecutionMessages());
						executionComments.append(Constants.Config.NEW_LINE)
								.append("Execution completed for the partfile ").append(ilSourceIterationCount)
								.append(" of ").append(ilSourceMultiPartCount);
						ilSourceIterationCount++;
					}
	...
	public String execureCurrencyLoad(JdbcTemplate clientAppDbJdbcTemplate, ClientJdbcInstance clientJdbcInstance,
			CurrencyIntegration currencyIntegration) {
		...
		List<ClientCurrencyMapping> clientCurrencyMappingList = etlAdminDao
				.getClientCurrencyMappingWithCurrencyIds(clientAppDbJdbcTemplate);
		...
		for (ClientCurrencyMapping clientCurrencyMapping : clientCurrencyMappingList) {
			...
			try {
				ClientDbCredentials clientDbCredentials = userDao
						.getClientDbDetails(clientCurrencyMapping.getClientId());
				...
				MinidwServiceUtil.getConcatinatedString(curreniesList,
		...
		try {
			...
			commonProcessor.moveErrorLogFile(dlParamsVals.get("CLIENT_ID").toString(),
					dlParamsVals.get("PACKAGE_ID").toString(), dlParamsVals.get("JOB_NAME").toString(),
					dlParamsVals.get("JOB_STARTDATETIME").toString(), dlParamsVals.get("ETL_JOB_ERROR_LOG").toString(),
					dlParamsVals.get("FILE_PATH").toString());

			ilStatus = etlJjobExecutionMessages.getStatus();
			executionmessages = etlJjobExecutionMessages.getErrorStreamMsg()
