...
public class StandardPackageProcessor {
	...
	public StandardPackageExecutor processStandardPackageExecutor(HttpServletRequest request, String encUserId,
			User user, DLInfo dlInfo, boolean isExecutionrequired) {

		S3BucketInfo s3BucketInfo = (S3BucketInfo) SessionHelper.getSesionAttribute(request,
				Constants.Config.S3_BUCKET_INFO);
		FileSettings fileSettings = (FileSettings) SessionHelper.getSesionAttribute(request,
				Constants.Config.FILE_SETTINGS_INFO);
		CustomRequest customRequest = getCustomRequest(request, user);

		return processStandardPackageExecution(user, dlInfo, s3BucketInfo, fileSettings, customRequest,
				isExecutionrequired);
	}

	public StandardPackageExecutor processStandardPackageExecutor(PackageExecution packageExecution,
			S3BucketInfo s3BucketInfo, FileSettings fileSettings, CustomRequest customRequest, String encUserId,
			User user, DLInfo dlInfo) {

		StandardPackageExecutor standardPackageExecutor = new StandardPackageExecutor(this, packageExecution, user,
				dlInfo, s3BucketInfo, fileSettings, customRequest);
		standardPackageExecutor.start();
		return standardPackageExecutor;
	}

	public void processStandardPackageExecution(PackageExecution packageExecution, User user, DLInfo dlInfo,
		...
		try {
			...
			if (runType.equals(Constants.jobType.IL) || runType.equals(Constants.jobType.ALL)) {
				metaDataFetch.updateUploadInfo(Constants.ExecutionStatus.INPROGRESS, "Source Upload started",
				...
				metaDataFetch.updateUploadInfo(Constants.ExecutionStatus.COMPLETED, "Source Upload Completed",
						packageExecution, customRequest);
			}
	...
	public void processStandardPackageExecution(PackageExecution packageExecution, User user, int dlId,
			S3BucketInfo s3BucketInfo, FileSettings fileSettings, CustomRequest customRequest) {
		DLInfo dlInfo = metaDataFetch.getSourcesByDlId(dlId, customRequest);
	...
	public StandardPackageExecutor processStandardPackageExecution(User user, DLInfo dlInfo, S3BucketInfo s3BucketInfo,
			FileSettings fileSettings, CustomRequest customRequest, boolean isExecutionrequired) {
		...
		return processStandardPackageExecutor(packageExecutionInfo, s3BucketInfo, fileSettings, customRequest,
				customRequest.getEncUserId(), user, dlInfo);
	...
	public CustomRequest getCustomRequest(HttpServletRequest request, User user) {
		...
		CustomRequest customRequest = new CustomRequest(webServiceContextUrl, user.getClientId(), user.getClientId(),
				browserDetails, deploymentType, null, user.getUserId());
		return customRequest;
	}

	public void runIlSources(User user, S3BucketInfo s3BucketInfo, FileSettings fileSettings,
			CustomRequest customRequest, PackageExecution packageExecution, Package userPackage, DLInfo dlInfo) {

		List<ILConnectionMapping> ilConnectionMappings = metaDataFetch.getPackageSourceMappingListByIds(customRequest,
				dlInfo, null);
		...
		if (ilConnectionMappings != null && ilConnectionMappings.size() > 0) {
			...
			metaDataFetch.updateUploadInfo(Constants.ExecutionStatus.INPROGRESS, noOfSourcesString,
					packageExecution, customRequest);
			int count = 0;
			for (ILConnectionMapping ilMappingInfo : ilConnectionMappings) {

				if (!ilMappingInfo.getIsFlatFile() && !ilMappingInfo.getIsWebservice()) {
					dataBaseProcessor.processDatabase(user, customRequest.getDeploymentType(), packageExecution,
							s3BucketInfo, fileSettings, customRequest, ilMappingInfo);
					...
					webServiceProcessor.processWebservice(user, userPackage, customRequest.getDeploymentType(),
							packageExecution, s3BucketInfo, fileSettings, ilMappingInfo, customRequest);
				}
				count++;
	...
	List<String> processDl(PackageExecution packageExecution, DLInfo dlInfo, CustomRequest customRequest) {
		...
		if (packageExecution.isJobExecutionRequired() && dlInfo.isDlExecutionRequired()) {
			Map<String, Object> dlprocessList = dlProcessor.processDL(dlInfo.getdL_id(), 0, packageExecution,
					customRequest);
			if (dlprocessList != null) {
				...
				throw new UploadandExecutionFailedException("Dl execution failed for package id : " + 0 + "dl id : "
						+ dlInfo.getdL_id() + "execution Id:" + packageExecution.getExecutionId());
			}
			...
			totalTablesList = dlProcessor.processCustomTableSets(0, ddLTableSet, ddLTableNamesSet,
					packageExecution, customRequest);
