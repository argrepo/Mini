...
public class WebServiceDataRestController {
	...
	public DataResponse getWebserviceTempleteDetails(@PathVariable(Constants.PathVariables.CLIENT_ID) String clientId,
			@PathVariable("templateId") Long wsTemplateId, HttpServletRequest request, Locale locale) {
		...
		try {
			...
			if (webServiceTemplateMaster != null) {
				...
				message.setText(messageSource
						.getMessage("anvizent.message.error.text.unableToRetrieveWSTamplateDetails", null, locale));
			}

		} catch (Throwable t) {
			packageService.logError(t, request, clientAppDbJdbcTemplate);
			message.setCode(com.anvizent.client.data.to.csv.path.converter.constants.Constants.Config.FAILED);
			message.setText(messageSource.getMessage("anvizent.message.error.text.unableToRetrieveWSTamplateDetails",
					null, locale));
	...
	public DataResponse getWebServiceAuthenticationTypes(
		...
		try {
			...
			if (webServiceAuthenticationTypes != null && webServiceAuthenticationTypes.size() > 0) {
				...
				message.setCode(com.anvizent.client.data.to.csv.path.converter.constants.Constants.Config.FAILED);
				message.setText(messageSource.getMessage("anvizent.message.error.text.listNotFound",
						null, locale));
	...
	public DataResponse saveWebserviceMasterConnectionMapping(
		...
		try {
			...
			if (webServiceConnectionMaster.getDataSourceName().equals("-1")) {
				ClientDataSources clientDataSource = new ClientDataSources(Long.parseLong(clientIdfromHeader),
						webServiceConnectionMaster.getDataSourceNameOther());
			...
			if (count > 0) {
				message.setCode(com.anvizent.client.data.to.csv.path.converter.constants.Constants.Config.SUCCESS);
				message.setText(messageSource.getMessage("anvizent.message.success.text.webServiceMasterConnectionSavedSuccessfully",
						null, locale));
				messages.add(message);
				dataResponse.setMessages(messages);
			} else {
	...
	public DataResponse getWebServiceConnections(@PathVariable(Constants.PathVariables.CLIENT_ID) String userId,
	...
	public DataResponse saveWebServiceTemplate(@PathVariable(Constants.PathVariables.CLIENT_ID) String clientId,
			@RequestBody WebServiceTemplateMaster webServiceTemplateMaster, HttpServletRequest request, Locale locale) {
		DataResponse dataResponse = new DataResponse();
		Message message = new Message();
		List<Message> messages = new ArrayList<>();
		int templateInfo = 0;
		...
		try {
			...
			if (templateInfo != 0) {
				...
				message.setText(messageSource
						.getMessage("anvizent.message.error.text.errorWhileSavingTheWebserviceTemplate", null, locale));
				messages.add(message);
				dataResponse.setMessages(messages);
			}
		} catch (AnvizentDuplicateFileNameException ae) {
			...
			message.setCode(com.anvizent.client.data.to.csv.path.converter.constants.Constants.Config.ERROR);
			message.setText(
					messageSource.getMessage("anvizent.message.error.text.webServiceNameAlreadyExist", null, locale));
			messages.add(message);
			dataResponse.setMessages(messages);
		} catch (Throwable t) {
			packageService.logError(t, request, clientAppDbJdbcTemplate);
	...
	public DataResponse testAuthenticateWebServiceApi(@PathVariable(Constants.PathVariables.CLIENT_ID) String clientId, @RequestBody WebServiceConnectionMaster webServiceConnectionMaster, HttpServletRequest request, Locale locale)
	{
		...
		try
		{
			...
			{
				response = WebServiceUtils.testAuthenticationUrl(webServiceConnectionMaster, authenticateRestTemplate);
				if( response != null )
				{
	...
	public DataResponse getWebServiceConnectionDetails(@PathVariable(Constants.PathVariables.CLIENT_ID) String clientId,
			@PathVariable("wsConId") Long wsConId, @PathVariable("ilId") Long ilId, HttpServletRequest request,
			Locale locale) {
		LOGGER.info("in getWebserviceTempleteDetails()");
		ResponseEntity<Map> response = null;
		ResponseEntity<String> soapResponse = null;
			...
			if (webServiceConnectionMaster != null) {
				...
				try {
					if(webServiceConnectionMaster.getWebServiceTemplateMaster().getWebserviceType().equals("SOAP")){
						...
						response = WebServiceUtils.testAuthenticationUrl(webServiceConnectionMaster, restTemplate);
						if (response != null) {
							statusCode = response.getStatusCode();
							if (statusCode != null && statusCode.equals(HttpStatus.OK)) {
	...
	public DataResponse getWebServiceConnectionDetailsById(
			@PathVariable(Constants.PathVariables.CLIENT_ID) String clientId, @PathVariable("wsConId") Long wsConId,
			HttpServletRequest request, Locale locale) {
		...
		try
		{
			...
			if( webService != null )
			{
				...
				if( webService.getWebServiceConnectionMaster().getId() != null )
				{
					...
					finalformattedApiResponse = WebServiceUtils.validateWebService(webService, connectionRestTemplate, null);
	...
	public DataResponse joinAndSaveWebServiceData(@PathVariable(Constants.PathVariables.CLIENT_ID) String clientId,
			@RequestBody WebService webService, Locale locale, HttpServletRequest request) {
		...
		try {
			...
			CommonUtils.deleteWebServiceIlConnectionMapping(user,clientId,  null, clientAppDbJdbcTemplate, webService,
					packageService.getPackageDao(), fileService.getFileDao(), modification);
			...
			if (webService.getWebServiceJoin().size() > 0) {
				...
				for (WebServiceJoin webServiceJoin : webService.getWebServiceJoin()) {
					...
					String incrementalDate = packageService.getILIncrementalUpdateDate(null, webService.getIlId(), "",Constants.SourceType.WEBSERVICE, clientStagingJdbcTemplate);
					...
					finalformattedApiResponse = WebServiceUtils.validateWebService(webServiceApi, connectionRestTemplate,null);
					...
					if (filePath != null) {
						...
						originalFileheaders = fileService.getHeadersFromFile(tempFile.getAbsolutePath(),
								Constants.FileType.CSV, ",", null);
						...
						if (originalFileheaders.size() > 0) {
							...
							CommonUtils.saveIlConnectionWebServiceMapping(filePath, originalFileheaders, clientId, clientStagingJdbcTemplate,
									clientAppDbJdbcTemplate, webService, packageService.getPackageDao(),
									fileService.getFileDao(), modification);
	...
	public DataResponse saveWebServiceData(@PathVariable(Constants.PathVariables.CLIENT_ID) String clientId,
			@RequestBody WebService webService, Locale locale, HttpServletRequest request) {
		...
		try {
			...
			if (webService != null) {
				...
				webServiceApi.setWebServiceConnectionMaster(wSService
						.getWebServiceConnectionDetails(Long.valueOf(webService.getWsConId()), clientIdfromHeader, clientAppDbJdbcTemplate));
			...
			if (filePath != null) {
				File tempFile = new File(filePath);
				originalFileheaders = fileService.getHeadersFromFile(tempFile.getAbsolutePath(), Constants.FileType.CSV,
						",", null);
				if (originalFileheaders.size() > 0) {
					...
					if (StringUtils.isNotBlank(iLInfo.getiL_table_name())) {
						...
						iLcolumnNames = packageService.getTableStructure(null, iLInfo.getiL_table_name(),
								webService.getIndustryId(), clientId, clientJdbcTemplate);
					}
				...
				message.setCode(com.anvizent.client.data.to.csv.path.converter.constants.Constants.Config.ERROR);
				message.setText(
						messageSource.getMessage("anvizent.message.error.text.unableToCallWebService", null, locale));
				messages.add(message);
				dataResponse.setMessages(messages);
				return dataResponse;
			}
			...
			message.setCode(com.anvizent.client.data.to.csv.path.converter.constants.Constants.Config.ERROR);
			message.setText(messageSource.getMessage("anvizent.message.error.text.errorWhileSavingWebserviceDetails",
					null, locale));
			messages.add(message);
			dataResponse.setMessages(messages);
			MinidwServiceUtil.getErrorMessage(message, "ERROR", t);
	...
	public DataResponse webservicePreviewStandard(@PathVariable(Constants.PathVariables.CLIENT_ID) String clientId,
			@PathVariable("webserviceConId") Integer webserviceConId, @PathVariable("il_id") Integer il_id,
			@PathVariable("mappingId") Integer mappingId, @PathVariable("packageId") Integer packageId, Locale locale, HttpServletRequest request) {
		...
		try {
			...
			if (webserviceConId != null && il_id != null) {
				webServiceApi = packageService.getIlConnectionWebServiceMapping(clientId, packageId, il_id,
						mappingId, clientAppDbJdbcTemplate);
			}

			webServiceApi.setWebServiceConnectionMaster(
					wSService.getWebServiceConnectionDetails(Long.valueOf(webserviceConId), clientIdfromHeader, clientAppDbJdbcTemplate));
			webServiceApi.setValidateOrPreview(true);
			clientJdbcTemplate = clientJdbcInstance.getClientJdbcTemplate(true);
			String incrementalDate = packageService.getILIncrementalUpdateDate(null, il_id,
					webServiceApi.getId().toString(), Constants.SourceType.WEBSERVICE, clientJdbcTemplate);
			webServiceApi.setInclUpdateDate(incrementalDate);
			...
			if (finalformattedApiResponse != null) {
				...
				message.setCode(com.anvizent.client.data.to.csv.path.converter.constants.Constants.Config.ERROR);
				message.setText(
						messageSource.getMessage("anvizent.message.error.text.apiProcessingFailed", null, locale));
				messages.add(message);
				dataResponse.setMessages(messages);
			}
	...
	public DataResponse editMappedHeadersForWebserviceApi(
			@PathVariable(Constants.PathVariables.CLIENT_ID) String clientId,
		...
		try {
			...
			if (StringUtils.isNotBlank(iLInfo.getiL_table_name())) {
				clientJdbcTemplate = clientJdbcInstance.getClientJdbcTemplate();
				iLcolumnNames = packageService.getTableStructure(null, iLInfo.getiL_table_name(), industryId, clientId,
						clientJdbcTemplate);
			}

			if (iLConnectionMapping.getConnectionMappingId() != null && iLConnectionMapping.getWsConId() != null) {
				mappedHeadersForWebservice = packageService.getMappedHeadersForWebservice(clientId,
						iLConnectionMapping.getiLId(), iLConnectionMapping.getConnectionMappingId(),
						iLConnectionMapping.getWsConId(), clientAppDbJdbcTemplate);
			}

			Map<String, String> mappedWebserviceHeaders = CommonUtils
					.mappedWebserviceHeaders(mappedHeadersForWebservice);

			File tempFile = null;
			if (!iLConnectionMapping.isJoinWebService()) {

				if (iLConnectionMapping.getWsConId() != null && iLConnectionMapping.getiLId() != null) {
					webServiceApi = packageService.getIlConnectionWebServiceMapping(clientId,
							iLConnectionMapping.getPackageId(), iLConnectionMapping.getiLId(),
							iLConnectionMapping.getConnectionMappingId(), clientAppDbJdbcTemplate);
				}
				webServiceApi.setWebServiceConnectionMaster(wSService.getWebServiceConnectionDetails(
						Long.valueOf(iLConnectionMapping.getWsConId()), clientIdfromHeader, clientAppDbJdbcTemplate));
				webServiceApi.setValidateOrPreview(true);
	...
	public DataResponse getwebServiceTemplate(@PathVariable(Constants.PathVariables.CLIENT_ID) String clientId,
			HttpServletRequest request, Locale locale) {
		DataResponse dataResponse = new DataResponse();
		List<Message> messages = new ArrayList<>();
		Message message = new Message();
		...
		try {
			...
			message.setCode(com.anvizent.client.data.to.csv.path.converter.constants.Constants.Config.ERROR);
			message.setText(
					messageSource.getMessage("anvizent.message.error.text.errorwhilegetWebServiceList", null, locale));
			messages.add(message);
			MinidwServiceUtil.getErrorMessage(message, "ERROR", t);
		} finally {
			CommonUtils.closeDataSource(clientAppDbJdbcTemplate);
	...
	public DataResponse getWebServiceTemplateById(@PathVariable(Constants.PathVariables.CLIENT_ID) String clientId,
			HttpServletRequest request, @RequestBody WebServiceTemplateMaster webServiceTemplateMaster, Locale locale) {
	...
	public DataResponse updateWebserviceMasterConnectionMapping(
			@PathVariable(Constants.PathVariables.CLIENT_ID) String clientId,
			@RequestBody WebServiceConnectionMaster webServiceConnectionMaster, HttpServletRequest request,
			Locale locale) {
		...
		try {
			...
			if (count > 0) {
				message.setCode(com.anvizent.client.data.to.csv.path.converter.constants.Constants.Config.SUCCESS);
				message.setText(messageSource.getMessage("anvizent.message.success.text.webServiceMasterConnectionMappingUpdatedSuccessfully",
						null, locale));
				messages.add(message);
				dataResponse.setMessages(messages);
			} else {
				message.setCode(com.anvizent.client.data.to.csv.path.converter.constants.Constants.Config.FAILED);
				message.setText(messageSource.getMessage("anvizent.message.error.text.WebserviceMasterConnectionMappingNotUpdatedSuccesfully",
						null, locale));
				messages.add(message);
				dataResponse.setMessages(messages);
			}
			...
			packageService.logError(t, request, clientAppDbJdbcTemplate);
			message.setCode(com.anvizent.client.data.to.csv.path.converter.constants.Constants.Config.FAILED);
			message.setText(messageSource.getMessage("anvizent.message.error.text.errorWhileUpdatingWebserviceMasterConnectionMapping",
					null, locale));
			messages.add(message);
			dataResponse.setMessages(messages);
			MinidwServiceUtil.getErrorMessage(message, "ERROR", t);
